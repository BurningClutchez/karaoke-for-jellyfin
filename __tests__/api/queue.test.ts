// DISABLED (kept for reference): these tests cover the REST queue handlers (src/app/api/queue/handlers),
// which is commented out. See __tests__/api/queue-readonly.test.ts for the
// read-only /api/queue that replaced it.
import { it } from "vitest";

it.todo("REST queue handlers (src/app/api/queue/handlers) is disabled");
